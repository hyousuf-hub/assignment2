...
@login_required
def product_list_function(request):
   product = Product.objects.filter(created_date__lte=timezone.now())
   return render(request, 'crm/products_list.html', {'products': product})

@login_required
def product_new_function(request):
   if request.method == "POST":
...
@login_required
def product_edit_function(request, pk):
...
@login_required
def product_delete_function(request, pk):
